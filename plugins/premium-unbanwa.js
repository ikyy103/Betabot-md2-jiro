...
let handler = async (m, { conn, usedPrefix, command, text }) => {
  ...
  if (!(texts[0] || texts[1]))
    return m.reply(
      `Masukan nama dan nomor WhatsApp \n\nContoh : \n${usedPrefix + command} ${conn.getName(m.sender)}|${m.sender.split("@")[0]}`,
    );
  if (new Date() - user.unbanwa < 3600000)
    return m.reply(
      `Silahkan menunggu selama ${timers}, untuk bisa menggunakan fitur ini kembali`,
    );
  ...
  const mailOptions = {
    ...
    text: textUnban.getRandom().replace("@", "+" + texts[1]),
  };
