class Blackjack {
    ...
    emitEndEvent() {
        ...
        clearTimeout(this.timer); // Clear the timer when the game ends
...
const templateBlackjackMessage = (usedPrefix, command, conn, m, blackjack) => {
    ...
    switch (state) {
        ...
        case "dealer_blackjack":
            ...
            break;
        case "time_up":
            message = `🕒 Waktu habis! Uang yang dipertaruhkan hangus. 😢`;
            global.db.data.users[conn.blackjack[m.chat].idPemain].money -= bet; 
            delete conn.blackjack[m.chat]; 
            break;
        default:
        ...
        switch (aksi) {
            ...
            case 'start':
                if (conn.blackjack[m.chat]) {
                    ...
                    if (global.db.data.users[m.sender].money < betAmount) {
                        await conn.reply(m.chat, 'Uang Anda tidak cukup untuk bertaruh. 😔', m);
                        break;
                    }
