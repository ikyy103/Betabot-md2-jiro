class Blackjack {
    ...
    start(user) {
        ...
        user.money -= this.table.bet; // Mengurangi uang pengguna
        ...
        this.updateTable();
        this.startTimer(); // Memulai timer
        return this.table;
    }

    startTimer() {
        this.timer = setTimeout(() => {
            this.state = "timeout"; // Set state timeout
            this.emitEndEvent();
        }, 60000); // 1 menit
    }
    ...
    emitEndEvent() {
        ...
        clearTimeout(this.timer); // Menghentikan timer
...
const templateBlackjackMessage = (usedPrefix, command, conn, m, blackjack) => {
    ...
    switch (state) {
        ...
        case "dealer_blackjack":
            ...
            break;
        case "timeout":
            message = `🕒 Waktu habis! Uang Anda hangus. 😢`;
            global.db.data.users[conn.blackjack[m.chat].idPemain].money -= bet; // Uang hangus
            delete conn.blackjack[m.chat];
            break;
        default:
            message = `🃏 • B L A C K J A C K •\n\n╭───┈ • │ Your Cards:\n│ ${playerCards} │ Your Total:\n│ ${playerTotal} ├───┈ • │ Dealer's Cards:\n│ ${hiddenDealerCards} │ Dealer's Total:\n│ ${dealerTotal > 21 ? 'BUST' : '❓'} ╰───┈ •\n\nBet:\n- ${formatter.format(bet)}\n\nType ${usedPrefix + command} hit to draw a card. Type ${usedPrefix + command} stand to end your turn.`;
            break;
    }
    return message;
}
        ...
        switch (aksi) {
            ...
            case 'start':
                if (conn.blackjack[m.chat]) {
                    ...
                    conn.blackjack[m.chat].start(global.db.data.users[m.sender]); // Mengirim user untuk mengurangi uang
