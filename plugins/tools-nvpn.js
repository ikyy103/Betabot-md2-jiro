...
const handler = async (m, { conn, args, usedPrefix, command }) => {
    ...
    if (command === 'nvpn' || command === 'nordvpn') {
        if (!subcommand) return conn.sendMessage(m.chat, { text: `Usage : *${usedPrefix}${command} File <file>/<url File*\n\n *${usedPrefix}${command} url* <url web>` }, { quoted: m });
        
        if (subcommand === 'file') {
            try {
                let q = m.quoted ? m.quoted : m;
                let mime = (q.msg || q).mimetype || '';
                if (!mime) throw 'Kirim atau balas file dengan command';

                let media = await q.download();
                if (!media) throw 'Gagal mengambil file';
...
*Is Safe :* ${result.result.isSafe ? 'Safe' : 'Dangerous'}`;
        ...
        if (subcommand === 'url') {
            try {
                const url = args[1];
                if (!url) throw `*Example :*${usedPrefix}${command} url https://googel.com`;
...
*Is Safe :* ${result.result.isSafe ? 'Safe' : 'Dangerous'}`;
        ...
        return conn.sendMessage(m.chat, { text: `Hanya Tersedia ${usedPrefix}${command} <file|url>` }, { quoted: m });
    }
};
...
handler.help = ['nvpn <file|url>', 'nordvpn <file|url>'];
handler.tags = ['security', 'tools'];
