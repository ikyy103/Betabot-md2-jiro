...
let handler = async (m, { conn, args, command, usedPrefix }) => {
    if (!args[0]) {
        return m.reply(`❌ Masukkan teks untuk video stiker!\n\nContoh: ${usedPrefix + command} BetaBotz dan Botcahx Menyala`);
    }

    const text = args.join(' '); // Gabungkan teks argumen
    const apiKey = '${lann}'; // Ganti dengan API key Anda
    const apiUrl = `https://api.betabotz.eu.org/api/maker/brat-video?text=${encodeURIComponent(text)}&apikey=${lann}`;
