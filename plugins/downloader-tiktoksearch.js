...
let handler = async (m, { conn, args, usedPrefix, command }) => {
  ...
  try {
    ...
    // Ambil video pertama dari hasil
    if (!api.result || !api.result.data || api.result.data.length === 0) {
      throw `🚩 *Video Tidak Ditemukan!*`;
    }

    let video = api.result.data[0]; // Ambil video teratas
    ...
    // Kirim video pertama dengan keterangan
