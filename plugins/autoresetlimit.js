...
function resetLimit(conn, lim) {
    ...
    conn.reply('120363347063028657@g.us', `*[ Informasi Limit ]*\n\n*Limit berhasil direset ${lim} / user*`, null); // Kirim info ke grup tertentu
}
...
function scheduleDailyReset(conn, lim) {
    ...
    autoResetTimeout = setTimeout(() => {
        if (isAutoResetEnabled) {
            console.log(`*[ Informasi Limit ]*\n\nMereset limit pengguna menjadi ${lim}`);
            resetLimit(conn, lim); 
            scheduleDailyReset(conn, lim); 
        }
    }, timeUntilMidnight); 
