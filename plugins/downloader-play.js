...
let handler = async (m, { conn, text, usedPrefix }) => {
    ...
    try {
        ...
        if (convert.seconds >= 3600) {
            ...
            caption += `∘ Viewers : ${convert.views || 0}\n`; // Pastikan views terdefinisi
            caption += `∘ Upload At : ${convert.ago}\n`;
            caption += `∘ Author : ${convert.author.name}\n`;
            caption += `∘ Channel : ${convert.author.url}\n`;
            caption += `∘ Url : ${convert.url}\n`;
            caption += `∘ Description : ${convert.description || 'No Description'}\n`; // Pastikan deskripsi terdefinisi
...
let handler = async (m, { conn, text }) => {
  ...
  try {
    ...
    let description = results.videos[0].description || 'No Description'; // Pastikan deskripsi terdefinisi
